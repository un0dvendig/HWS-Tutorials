...
class BuildingNode: SKSpriteNode {
    ...
    func hit(at point: CGPoint) {
        let convertedPoint = CGPoint(x: point.x + size.width / 2,
        ...
        let img = renderer.image { context in
            ...
            context.cgContext.addEllipse(in: CGRect(x: convertedPoint.x - 32, y: convertedPoint.y - 32,
                                                    width: 64, height: 64))
        ...
        let img = renderer.image { (context) in
            ...
            for row in stride(from: 10, to: Int(size.height - 10), by: 40) {
                for col in stride(from: 10, through: Int(size.width - 10), by: 40) {
                    ...
                    context.cgContext.fill(CGRect(x: col, y: row,
                                                  width: 15, height: 20))
