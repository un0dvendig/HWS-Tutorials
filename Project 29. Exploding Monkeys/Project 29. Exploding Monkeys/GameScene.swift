...
class GameScene: SKScene,
    ...
    func launch(angle: Int, velocity: Int) {
        ...
        if currentPlayer == 1{
            ...
            let impulse = CGVector(dx: cos(radians) * speed,
    ...
    private func destroy(player: SKSpriteNode) {
        if let explostion = SKEmitterNode(fileNamed: "hitPlayer") {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
            ...
            if player.name == "player1" {
                ...
                playerScore = self.viewController!.player2Score
            } else {
                self.viewController?.player1Score += 1
                playerScore = self.viewController!.player1Score
            }
            
            guard playerScore < 3 else {
    ...
    private func addWind() {
        let randomDX = CGFloat.random(in: -10...10)
        let randomDY = CGFloat.random(in: (-9.8)...(-4))
        physicsWorld.gravity = CGVector(dx: randomDY,
                                        dy: randomDY)
        DispatchQueue.main.async { [weak self] in
            self?.viewController?.updateWindLabel(dx: randomDX,
                                                  dy: randomDY)
        }
    }
